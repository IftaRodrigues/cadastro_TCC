 ...
 * @author Idê Moraes
 */
public class ExcluirBanca extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int id_banca = Integer.parseInt(request.getParameter("id_banca"));

            banca.setId_banca(id_banca);

            try {
                ServiceBanca.removeBanca(banca);
                response.sendRedirect("listaBanca.jsp");

            } catch (Exception e) {
                out.println("<h2><script>alert(\">Erro ao remover Banca!\");</script></h2>");
